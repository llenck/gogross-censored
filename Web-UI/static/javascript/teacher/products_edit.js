document.addEventListener("DOMContentLoaded", () => {
  ...
  var product_id = new URLSearchParams(window.location.search).get("product");
  ...
  function submitProduct(ev) {
    ...
    const productInfo = {
      ...
      category: document.querySelector("#category + .form-control").value,
    ...
    if (uploadedImageBase64) {
      ...
      if (img_sz_mb > 3) {
        if (
          !confirm(
            "You're about to upload an image of size " +
              Math.round(img_sz_mb * 10) / 10 +
              "MiB. Are you sure that you want to burden your students' bandwidth with this rather large file?"
          )
        ) {
          return;
        }
  ...
  function sendProductInfo(productInfo) {
    ...
    fetch("/products/edit?shop=" + shop_id + "&product=" + product_id,{
      method: "POST",
      headers: {
